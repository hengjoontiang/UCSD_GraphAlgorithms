...
struct Node {
	...
	//define a matrix to store the weights, using -1 to indicate no path
	//use 
	
	//try to use a map for storgae so that we still get a key
	vector<vector<std::map<int,int>>> adj_cost;
	...
	void printNode(){
		...
		for (int i=0;i<adj_cost.size();i++) {
			for (int x=0; x < adj_cost[i].size();x++){
				...
				std::cout << " nextIndex=" << nextIndex << " weight=" << weight;
			}
			std::cout << std::endl;
		}
	}*/
...
int distance(Node & node, int s, int t) {
  ...
  for (int index=1; index <= node.getNumNodes(); index++) {
	  ...
	  /*
	  //now we do a scan through the nodes
	  for (int subindex=1; subindex <= node.getNumNodes(); subindex++) {
		  ...
		  for (int adj_index= 1; adj_index <=  node.getLinkedVectorSize(current_index) ; adj_index++ ) {
			   ...
			   if (node.getIsVisited(nextIndex)) continue; 
			    ...
			    if (dist[u]+node.getWeight(indexwifMinDist,v) < dist[v]) 
		  }
	  }*/
  }
